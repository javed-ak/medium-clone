...
export default function Auth({ type }: { type: "signup" | "signin" }) {
  ...
  async function sendRequest() {
    try{
      const response = await axios.post(`${BACKEND_URL}/api/v1/user/${type === 'signup' ? 'signup' : 'signin'}`, inputValues);
  ...
  return (
    <div className="w-full h-full lg:w-1/2 flex lg:flex-col justify-center items-center">
      <div className="w-80">
        ...
        <div>
          {type === "signup" ? <LabelInput label="Name" type="text" placeholder="John Duo." onChange={(e) => {
            setInputValues({
              ...inputValues,
              name: e.target.value
            })
          ...
          <button onClick={sendRequest} className="w-full bg-slate-900 text-slate-50 rounded-lg p-2 mt-4">
